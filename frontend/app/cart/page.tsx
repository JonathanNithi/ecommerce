...
import { GET_PRODUCTS_BY_IDS } from "@/graphql/queries/product-queries"; // Import the new query
import { checkStockAndProceed } from '@/lib/check-stock';
import UnavailableProductsModal from "@/components/modals/UnavailableProductsModal";
...
export default function CartPage() {
  ...
  const { data: productsDataWithStock, loading: availabilityLoading, error: availabilityError } = useQuery(
    GET_PRODUCTS_BY_IDS, // Use the new query
    {
  ...
  const handleCheckout = useCallback(async () => {
    ...
    const stockCheckResult = await checkStockAndProceed(
      ...
      clearCart,
      router,
      productsDataWithStock?.productsById // Pass the data with stock information
    );
    ...
    if (stockCheckResult && stockCheckResult.insufficientStock.length > 0) {
      ...
      // Order was handled within checkStockAndProceed
  ...
  }, [isAuthenticated, accountId, accessToken, refreshToken, items, clearCart, createOrder, router, setLastOrder, client, productsDataWithStock?.productsById]);
